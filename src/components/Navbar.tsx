...
const Navbar = () => {
  ...
  const NavigationLinks = ({ mobile = false }: { mobile?: boolean }) => {
    ...
    return (
      <TooltipProvider>
        <div className="flex items-center gap-2 lg:gap-8 md:justify-between md:flex-1 md:px-2 md:max-w-2xl md:mx-auto">
          {navigationItems.map((item) => {
            // Special handling for "Gravar Aula" - opens modal instead of navigation
            if (item.href === '__modal__') {
  ...
  return (
    <>
      <header className="fixed top-0 z-[100] w-full border-b bg-white shadow-sm [transform:translateZ(0)] will-change-transform">
