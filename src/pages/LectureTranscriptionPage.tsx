          ...
          {structuredContent && (
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_380px] xl:grid-cols-[minmax(0,1fr)_420px] gap-4 md:gap-6">
              ...
              <div className="space-y-6">
                ...
                {/* Quick Access Cards - Preview de Conteúdo Gerado */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  ...
                  <Card className="bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200 hover:shadow-lg transition-shadow cursor-pointer"
                        onClick={() => {
                          const resumoTab = document.querySelector('[value="resumo"]') as HTMLElement;
                          if (resumoTab) {
                            resumoTab.click();
                            resumoTab.scrollIntoView({ behavior: 'smooth', block: 'center' });
                          }
                        }}>
                      ...
                      {lecture?.raw_transcript ? (
                        <>
                          <p className="text-xs text-slate-700 line-clamp-3 mb-2">
                            {lecture.raw_transcript.substring(0, 200)}...
                          </p>
                          <Badge className="bg-blue-100 text-blue-800 text-xs">
                            {Math.ceil(lecture.raw_transcript.split(' ').length / 150)} min de leitura
                          </Badge>
                        </>
                      ) : (
                        <p className="text-xs text-slate-500 italic">Nenhuma transcrição disponível</p>
                      )}
                    </CardContent>
                  </Card>
                      ...
                      {hasQuiz ? (
                        <>
                          <p className="text-2xl font-bold text-purple-700 mb-1">
                            {generatedQuiz?.questions?.length || structuredContent?.perguntas_revisao?.length || 0}
                          </p>
                          <p className="text-xs text-slate-600">perguntas geradas</p>
                      ...
                      {hasFlashcards ? (
                        <>
                          <p className="text-2xl font-bold text-green-700 mb-1">
                            {generatedFlashcards?.cards?.length || structuredContent.flashcards.length}
                          </p>
                          <p className="text-xs text-slate-600">cards gerados</p>
                          <Button size="sm" variant="outline" className="mt-2 w-full border-green-300 text-green-700 hover:bg-green-100">
                            <Eye className="h-3 w-3 mr-1" />
                            Ver Flashcards
                          </Button>
                        </>
                      ) : (
                        <p className="text-xs text-slate-500 italic">Nenhum flashcard disponível</p>
                      )}
                    </CardContent>
                  </Card>
                </div>
