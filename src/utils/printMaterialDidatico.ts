...
// ===== HELPER: Extrair seções individuais do DOM =====
const extractSections = (element: HTMLElement): HTMLElement[] => {
  const sections: HTMLElement[] = [];
  
  // Selecionar todos os elementos principais (títulos, parágrafos, callouts, fórmulas, listas, tabelas)
  const selectors = [
    'h2', 'h3', 'h4', 
    'p:not(.callout-content p):not(blockquote p)',
    '[data-callout-type]',
    '.katex-display',
    'ul:not(.callout-content ul)',
    'ol:not(.callout-content ol)',
    'table',
    'pre'
  ];
  
  element.querySelectorAll(selectors.join(', ')).forEach(el => {
    // Evitar duplicatas (elementos dentro de callouts já são capturados com o callout)
    if (!el.closest('[data-callout-type]') || el.hasAttribute('data-callout-type')) {
...
// ===== HELPER: Capturar seção individual como imagem otimizada =====
const captureSectionAsImage = async (
  section: HTMLElement,
  maxWidthMM: number
): Promise<ImageData> => {
  ...
  try {
    // Capturar com html2canvas (scale reduzido para otimização)
    const canvas = await html2canvas(container, {
      scale: 1.2, // Reduzido de 2 para 1.2 → economia de 44%
    ...
    // Compressão inteligente: JPEG para texto, PNG para callouts/fórmulas
    const hasColor = clonedSection.hasAttribute('data-callout-type') || 
                     clonedSection.classList.contains('katex-display') ||
                     clonedSection.tagName === 'TABLE';
    
    const format: 'JPEG' | 'PNG' = hasColor ? 'PNG' : 'JPEG';
    const quality = hasColor ? 0.92 : 0.85;
    
    const base64 = canvas.toDataURL(`image/${format.toLowerCase()}`, quality);

    // Calcular dimensões em mm (10 pixels/mm para boa qualidade)
    const widthMM = maxWidthMM * 0.95;
    const heightMM = (canvas.height / canvas.width) * widthMM;

    return { base64, format, width: canvas.width, height: heightMM };
  } finally {
    document.body.removeChild(container);
  }
};

// ===== FUNÇÃO PRINCIPAL: Gerar PDF Otimizado =====
export const printMaterialDidatico = async (
  ...
  console.log('🖨️ Iniciando geração de PDF otimizado...');
  ...
  // ===== FASE 1: Logo NextClass centralizada =====
...
<defs>
<linearGradient id="paint0_linear_2102_8238" x1="46.0757" y1="4.76111" x2="46.0757" y2="26.0005" gradientUnits="userSpaceOnUse">
<stop stop-color="#E879F9"/>
<stop offset="1" stop-color="#A855F7"/>
</linearGradient>
<linearGradient id="paint1_linear_2102_8238" x1="91" y1="0" x2="91" y2="10.5" gradientUnits="userSpaceOnUse">
<stop stop-color="#E879F9"/>
<stop offset="1" stop-color="#A855F7"/>
</linearGradient>
<linearGradient id="paint2_linear_2102_8238" x1="101.805" y1="6.15525" x2="101.805" y2="9.47528" gradientUnits="userSpaceOnUse">
<stop stop-color="#E879F9"/>
<stop offset="1" stop-color="#A855F7"/>
</linearGradient>
<linearGradient id="paint3_linear_2102_8238" x1="101.805" y1="9.8845" x2="101.805" y2="14.2367" gradientUnits="userSpaceOnUse">
<stop stop-color="#E879F9"/>
<stop offset="1" stop-color="#A855F7"/>
</linearGradient>
</defs>
  ...
  try {
    const logoPNG = await convertSVGtoPNG(nextclassLogoSVG, 752, 108);
    const logoWidthMM = 40;
    const logoHeightMM = (108 / 752) * logoWidthMM;
    const logoX = (pageWidth - logoWidthMM) / 2;
    
    pdf.addImage(logoPNG, 'PNG', logoX, currentY, logoWidthMM, logoHeightMM);
    currentY += logoHeightMM + 10;

    // ===== FASE 2: Título + Linha Gradiente =====
    pdf.setFontSize(18);
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(23, 15, 73); // NextClass dark color
    pdf.text(options.lectureTitle, pageWidth / 2, currentY, { align: 'center' });
    currentY += 5;

    // Linha gradiente (simulada com linhas rosa→roxo)
    const gradientY = currentY;
    const segmentWidth = contentWidth / 20;
    for (let i = 0; i < 20; i++) {
      const ratio = i / 19;
      const r = Math.round(232 + (168 - 232) * ratio); // 232 (pink) → 168 (purple)
      const g = Math.round(121 + (85 - 121) * ratio);
      const b = Math.round(249 + (247 - 249) * ratio);
    ...
    // ===== FASE 3: Capturar e adicionar seções =====
    const sections = extractSections(element);
    console.log(`📄 Capturando ${sections.length} seções...`);

    for (let i = 0; i < sections.length; i++) {
      const section = sections[i];
      
      try {
        const imageData = await captureSectionAsImage(section, contentWidth);
        
        // Verificar se cabe na página atual
        const availableSpace = pageHeight - currentY - FOOTER_HEIGHT - 5;
        const willFit = imageData.height <= availableSpace;
        ...
        currentY += imageData.height + 6;
        
        // Log de progresso
        if ((i + 1) % 5 === 0) {
          console.log(`✅ Progresso: ${i + 1}/${sections.length} seções`);
        }
      } catch (error) {
        console.warn(`⚠️ Erro ao capturar seção ${i + 1}:`, error);
      }
    }

    // ===== FASE 4: Footer em todas as páginas =====
    const totalPages = pdf.internal.pages.length - 1;
    console.log(`📝 Adicionando footer em ${totalPages} páginas...`);
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      // Linha gradiente footer
      const footerLineY = footerY - 5;
      for (let i = 0; i < 20; i++) {
        const ratio = i / 19;
        const r = Math.round(168 + (232 - 168) * ratio);
        const g = Math.round(85 + (121 - 85) * ratio);
        const b = Math.round(247 + (249 - 247) * ratio);
        pdf.setDrawColor(r, g, b);
      ...
      // Esquerda: Título (rosa)
      pdf.setTextColor(232, 121, 249);
      const truncatedTitle = options.lectureTitle.length > 40 
        ? options.lectureTitle.substring(0, 37) + '...'
        : options.lectureTitle;
      pdf.text(truncatedTitle, margin, footerY);
      
      // Centro: Página
      pdf.setTextColor(107, 33, 168);
      pdf.text(`${pageNum} / ${totalPages}`, pageWidth / 2, footerY, { align: 'center' });
    ...
    console.log(`📦 Seções capturadas: ${sections.length}`);
